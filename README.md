...
## Where trained model files are now

You can find them in:
- `backend/trained_models/model-manifest.json`
- `backend/trained_models/artifacts/*.json`

These are packaged training artifacts/metadata used directly by the simulator catalog. For final submission, replace them with real binary checkpoints (`.pkl`, `.joblib`, `.onnx`, `.pt`) while keeping the same manifest structure.
